...
with st.expander("📖 How to Use", expanded=False):
    ...
    **Follow these simple steps:**
    ...
    3. 💬 **Ask** questions about your research in the chat below
    4. 📚 **Get** answers with exact page citations!
    5. 💾 **All queries are saved** in database for future reference
    """)
...
def generate_tree_diagram(chunks, sources):
    ...
    if not sections:
        return "🌲 **No clear document structure detected.**\n\nThe PDF may not have recognizable section headings."
    ...
    tree_lines.append("")
    tree_lines.append("```")
    tree_lines.append("📑 DOCUMENT STRUCTURE TREE")
    ...
    tree_lines.append(f"- 📄 File: {filename}")
    tree_lines.append(f"- 📖 Sections found: **{len(sections)}**")
    tree_lines.append(f"- 📁 Main sections: **{len(main_sections)}**")
...
with st.sidebar:
    ...
    if st.button("🚀 Process Documents", type="primary", use_container_width=True):
        if uploaded_files:
            ...
            status_text.text("✅ Processing complete! Building search index...")
    ...
    if st.button("🌲 Show me the tree diagram", use_container_width=True):
    ...
    if st.session_state.show_history:
        ...
        if history:
                ...
                with st.expander(f"📝 {question[:40]}..."):
                    ...
                    with col_a:
                        ...
                        if st.button(star_label, key=f"bookmark_{q_id}"):
                            ...
                            st.rerun()
                    with col_b:
                        if st.button("🗑️ Delete", key=f"delete_{q_id}"):
                            delete_query(q_id)
                            st.rerun()
                    with col_c:
            ...
            st.info("No queries yet. Ask a question!")
    
    # Show bookmarked queries
    if st.session_state.show_bookmarks:
        ...
        if bookmarks:
            ...
            st.info("No bookmarked queries. Click ⭐ on any answer to bookmark!")
...
# Chat input with quick questions
...
if prompt:
    # Check if user wants a tree diagram
...
# Footer
st.markdown("---")
st.markdown("✨ **100% Free** - Powered by Google Gemini | 💾 All queries saved to database")